...
class PriceView: UIView {
    ...
    func update(ticker: TickerProtocol) {
        ...
        let changePrice = ticker.signed_change_price.formattedStringWithCommaAndDecimal(places: 5)
        let changeRate = ticker.signed_change_rate * 100
        
        // MARK: 현재가 업데이트
        self.priceLabel.text =  "₩\(ticker.trade_price.formattedStringWithCommaAndDecimal(places: 5))"
